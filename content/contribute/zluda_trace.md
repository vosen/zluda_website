...
an archive with your logs from `<LOG_DIRECTORY>` to attach to the issue:
...
`LD_LIBRARY_PATH` is an environment variable used by `ld`, Linux's dynamic
linker. It tells `ld` that when it's looking for a shared library – for
example, `libcuda.so` – it should look in a specific list of directories
rather than the system defaults. It's just like `PATH`, but for shared
...
As you can see, this is the add function from `add.cu`.
...
We use this information to discover which PTX instructions we need to
implement.
